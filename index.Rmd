...
#   mutate(INCDTTM = parse_date_time(INCDTTM, orders = c('%m/%d/%Y %H:%M:%S %p'),
#                                    tz = 'America/Los_Angeles'),
#          INCHOUR = hour(INCDTTM)) %>%
#   filter(!is.na(INCHOUR)) %>% 
#   group_by(INCDATE, INCHOUR) %>%
#   summarize(CNT = n()) %>% ungroup() %>% 
#   group_by(INCHOUR) %>% 
#   summarise(MIN = min(CNT), MEAN = mean(CNT), MAX = max(CNT))
# 
# tltip <- tooltip_table(c("Min", "Mean", "Max"), sprintf("{point.%s}", c("MIN", "MEAN", "MAX")))
# 
# hchart(y, type = "columnrange",
#        hcaes(x = INCHOUR, low = MIN, high = MEAN,
#              color = MEAN)) %>%
#   hc_chart(polar = TRUE) %>%
#   hc_yAxis(max = 3, min = 1, labels = list(format = "{value}"),
#             showFirstLabel = FALSE) %>%
#   hc_xAxis(
#     title = list(text = ""), gridLineWidth = 0.5,
#     labels = list(format = "{value}:00")) %>%
#   hc_tooltip(useHTML = TRUE, pointFormat = tltip,
#              headerFormat = as.character(tags$small("{point.x}:00")))
# frameWidget(l, height = 250)
```
]]
